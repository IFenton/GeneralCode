...
## Isabel Fenton
##
## Code to allow different palettes in vis.gam
## 
## functions:
## vis.gam.col - different colours palettes

# vis.gam.col -------------------------------------------------------------
# vis gam colour
vis.gam.col <- function (x, view = NULL, cond = list(), n.grid = 30, too.far = 0, 
                       ...
                       plot.type = "persp", zlim = NULL, nCol = 50, ...) 
{
  ...
  if (se <= 0) {
    ...
    else if (color == "rev.heat") {
      pal <- rev(heat.colors(nCol))
      con.col <- 3
    }
    else if (color == "log.heat") {
      pal <- log.heat(nCol)
      con.col <- 3
    }
    else if (color == "topo") {
